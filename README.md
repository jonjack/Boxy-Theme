...
2. Since the original theme is no longer available via the [Package Control central repository](https://packagecontrol.io/packages/Boxy%20Theme) you need to add this repository. In ST go to Tools > Command Palette > type "Package Control" and choose "Package Control: Add Repository" and paste this repo URL https://github.com/jonjack/sublime-boxy
3. Now select "Package Control: Install Package" and type "boxy". You should see the "Boxy-Theme" package, select it to install.
4. Without closing ST, go to the package installation directory. On OSX this is usually at ~/Library/Application Support/Sublime Text X. Make the following changes:-
  a. Rename Installed Packages/Boxy-Theme.sublime-package to remove the hyphen so it becomes Installed Packages/Boxy Theme.sublime-package.
  b. Edit the theme name in Packages/User/Package Control.sublime-settings to remove the hyphen so it looks as below. Save the change.
  ...
  ```
5. Restart ST.
6. Go to Preferences > Select Theme. If all went well you should see the options for Boxy in the list. If not, re-check the above steps.
